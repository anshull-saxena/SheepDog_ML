...
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # Fallback to CPU if CUDA is unavailable
...
def synonym_swap(text, n=1):
    ...
    # Identify nouns (NN, NNS, NNP, NNPS) and adjectives (JJ, JJR, JJS)
...
def create_train_loader(contents, contents_aug1, contents_aug2, labels, fg_label, aug_fg1, aug_fg2, tokenizer, max_len, batch_size):
    ...
    return DataLoader(ds, batch_size=batch_size, shuffle=True, num_workers=0)  # Set num_workers to 0 for compatibility
...
def train_model(tokenizer, max_len, n_epochs, batch_size, datasetname, iter):
    ...
    torch.save(model.state_dict(), os.path.join('checkpoints', f'{datasetname}_synonym_iter{iter}.m'))
...
def main():
    ...
    with open(log_file, 'a+') as f:
        ...
        f.write(f'Average Prec/Rec/F1 (macro): {sum(prec_all)/iterations:.4f}, '
                f'{sum(rec_all)/iterations:.4f}, {sum(f1_all)/iterations:.4f}\n')
        ...
        f.write(f'Average Prec/Rec/F1 (macro): {sum(prec_all_a)/iterations:.4f}, '
                f'{sum(rec_all_a)/iterations:.4f}, {sum(f1_all_a)/iterations:.4f}\n')
        ...
        f.write(f'Average Prec/Rec/F1 (macro): {sum(prec_all_b)/iterations:.4f}, '
                f'{sum(rec_all_b)/iterations:.4f}, {sum(f1_all_b)/iterations:.4f}\n')
        ...
        f.write(f'Average Prec/Rec/F1 (macro): {sum(prec_all_c)/iterations:.4f}, '
                f'{sum(rec_all_c)/iterations:.4f}, {sum(f1_all_c)/iterations:.4f}\n')
        ...
        f.write(f'Average Prec/Rec/F1 (macro): {sum(prec_all_d)/iterations:.4f}, '
                f'{sum(rec_all_d)/iterations:.4f}, {sum(f1_all_d)/iterations:.4f}\n')
        ...
        f.write(f'Average Prec/Rec/F1 (macro): {sum(prec_all_res)/iterations:.4f}, '
                f'{sum(rec_all_res)/iterations:.4f}, {sum(f1_all_res)/iterations:.4f}\n')
